...
from src.database.generate_proxy_database import (
    STATE_RTO_MAP, create_database_schema
)
from src.database.manager import DBManager

PATTERN_INDIA = re.compile(r'([A-Z]{2})([0-9]{1,2})([A-Z]{1,3})([0-9]{1,4})')
DB_PATH = "arg_master_database.sqlite" # Handled by Manager now
...
class EvidenceCollector:
    ...
    def __init__(self, output_dir=None):
        from src.utils.config import EVIDENCE_VAULT
        self.output_dir = output_dir or str(EVIDENCE_VAULT)
...
def generate_legal_narrative(plate_text, violation_type, vehicle_type,
    ...
    if plate_text != "UNKNOWN":
        try:
            ...
            if conn:
                q = """SELECT v.make, v.model, v.color, v.vehicle_class, v.invoice_price,
                ...
                row = DBManager.fetch_one(q, (plate_text,))
                if row:
                    vehicle_desc = f"{row['color']} {row['make']} {row['model']} ({row['vehicle_class']})"
                    vehicle_value = row['invoice_price'] or 500000
                    insurance = row['insurance_company'] or "Unknown"
                    financer = f"{row['hypothecation']} - {row['financer_bank']}" if row['hypothecation'] == "Yes" else "No active loan"
                    owner_name = row['owner_name'] or "Unknown Owner"
                    aadhar = row['owner_aadhar'] or "XXXX XXXX XXXX"
                    state = row['state'] or "Unknown"
                conn.close()
        except Exception as e:
            print(f"Narrative Lookup Error: {e}")
...
def run_video_engine(source, select_roi=False):
    ...
    # Output video
    from src.utils.config import PROCESSED_DATA_DIR; output_dir = str(PROCESSED_DATA_DIR / "arg_engine_output")
    os.makedirs(output_dir, exist_ok=True)
