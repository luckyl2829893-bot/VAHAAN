...
# ── DB path — auto-resolves to project root ───────────────────────────────── #
ROOT    = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(ROOT, "arg_master_database.sqlite")
...
# ══════════════════════════════════════════════════════════════════════════════
#  SCHEMA  — uses EXACT column names from your existing production DB
# ══════════════════════════════════════════════════════════════════════════════
...
def ensure_schema(conn):
    ...
    # citizens — 13 columns (unchanged from original)
    c.execute("""
        CREATE TABLE IF NOT EXISTS citizens (
            aadhar_masked   TEXT PRIMARY KEY,
    ...
    # vahan_registry — 21 columns, EXACT original names
    c.execute("""
    ...
    # fastag_accounts — 11 columns (unchanged from original)
    c.execute("""
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS fastag_transactions (
            ...
            FOREIGN KEY (fastag_id) REFERENCES fastag_accounts (fastag_id)
        )
    """)
...
def plate_exists(conn, plate_number):
    return conn.execute(
        "SELECT 1 FROM vahan_registry WHERE plate_number=? LIMIT 1",
        (plate_number,)
...
def _ins_citizen(c, aadhar, vid, cit):
    ...
    """, {
        "aadhar_masked": aadhar, "virtual_id": vid,
        "full_name": cit["name"], "gender": cit["gender"],
        "dob": cit["dob"],        "address": cit["addr"],
        "city": cit["city"],      "state": cit["state"],
        "phone": cit["phone"],    "pan": cit["pan"],
        "cibil": cit["cibil"],    "kyc": "Full KYC",
    })
...
def _ins_vahan(c, plate, veh, cit, aadhar,
    ...
    c.execute("""
        INSERT OR IGNORE INTO vahan_registry (
            ...
            :plate,            :vclass,          :make,            :model,
            :color,            :fuel,            :chassis,         :engine,
            :reg_date,         :rto,             :rc_status,       :fitness,
            :price,            :ins_co,          :ins_pol,         :ins_exp,
            :puc,              :hypo,            :financer,        :aadhar,
            :owner
        )
    """, {
        "plate":     plate,            "vclass":   veh["class"],
        "make":      veh["make"],      "model":    veh["model"],
        "color":     veh["color"],     "fuel":     veh["fuel"],
        "chassis":   _chassis(),       "engine":   _engine(),
        "reg_date":  reg_date,         "rto":      f"RTO {rto_city}",
        "rc_status": "Active",         "fitness":  fitness,
        "price":     veh["price"],     "ins_co":   ins_co,
        "ins_pol":   ins_pol,          "ins_exp":  ins_exp,
        "puc":       puc,              "hypo":     hypo,
        "financer":  financer,         "aadhar":   aadhar,
        "owner":     cit["name"],
    })
...
def _ins_fastag(c, fid, tid, plate, wallet, issuer, bank, status, vc):
    c.execute("""
        INSERT OR IGNORE INTO fastag_accounts (
            ...
            :fid, :tid, :plate, :cid,
            :wallet, :issuer, :bank, :status,
            :vc, 'Full KYC', :low
        )
    """, {
        "fid": fid, "tid": tid, "plate": plate,
        "cid": f"CID-{random.randint(10000,99999)}",
        "wallet": wallet, "issuer": issuer, "bank": bank,
        "status": status, "vc": vc,
        "low": 1 if wallet < 100 else 0,
    })
...
def _ins_transactions(c, fid):
    for tx in _transactions():
        c.execute("""
            ...
            VALUES (:fid, :plaza, :ts, :amt)
        """, {"fid": fid, "plaza": tx["plaza"], "ts": tx["ts"], "amt": tx["amt"]})
    ...
    for _ in range(random.randint(1,3)):
        ...
        c.execute("""
            INSERT INTO challans (
                ...
                :plate, :viol, :base,
                :mult,  :final, :status, :date
            )
        """, {
            "plate": plate, "viol": viol, "base": base,
            "mult": mult, "final": round(base*mult,2),
            "status": random.choice(["Unpaid","Unpaid","Paid"]),
            "date": _rdate(2024, 2026),
        })
